"""
HistGradientBoosting Model for Taxi Trajectory Prediction
- Scikit-learn's fast histogram-based gradient boosting
...
def train_histgb_model():
    """Train HistGradientBoosting models for longitude and latitude"""
    print("=" * 60)
    print("HistGradientBoosting Model Training")
    print("=" * 60)
    ...
    model_lon = HistGradientBoostingRegressor(**hgb_params)
    model_lon.fit(X_train, y_train.iloc[:, 0])
    
    # Train latitude model
    print("\n3. Training Latitude Model...")
    model_lat = HistGradientBoostingRegressor(**hgb_params)
    model_lat.fit(X_train, y_train.iloc[:, 1])
    ...
    print("\n--- Training Set ---")
    train_metrics = evaluate_predictions(y_train, y_train_pred, "HistGradientBoosting")
    for k, v in train_metrics.items():
        print(f"   {k}: {v:.6f}")
    
    print("\n--- Validation Set ---")
    val_metrics = evaluate_predictions(y_val, y_val_pred, "HistGradientBoosting")
    for k, v in val_metrics.items():
        print(f"   {k}: {v:.6f}")
    
    # Save models
    print("\n6. Saving models...")
    joblib.dump(model_lon, "models/histgb/histgb_lon.pkl")
    joblib.dump(model_lat, "models/histgb/histgb_lat.pkl")
    ...
    with open("models/histgb/metrics_histgb.json", "w") as f:
        json.dump(all_metrics, f, indent=2)
    
    print("\n✓ HistGradientBoosting models saved successfully!")
    print("=" * 60)
